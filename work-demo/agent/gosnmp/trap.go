...
func (t *TrapListener) SendUDP(packet *SnmpPacket, addr *net.UDPAddr) error {
	...
	if count != len(ob) {
		log.Info("Failed to send all bytes of SnmpPacket!\n")
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				log.Info("TrapListener: error in read \n", errors.New(err.Error()))
			...
			if err != nil {
				log.Info("TrapListener: error in UnmarshalTrap \n", errors.New(err.Error()))
			...
			if trap.Version == Version3 && trap.SecurityModel == UserSecurityModel && t.Params.SecurityModel == UserSecurityModel {
				...
				if !ok {
					log.Info("TrapListener: Invalid SecurityParameters types  \n")
...
func (t *TrapListener) debugTrapHandler(s *SnmpPacket, u *net.UDPAddr) {
	log.Info("got trapdata from %+v: %+v\n", u, s)
}
